...
class EVTemplate {
  ...
  *
  * echo EVTemplate::render('hello', array('name' => 'World'));
  *
  * will output:
  *
  * <h1>Hello, World!</h1>
  ...
  */
  ...
  public static function render($template, $vars = array()) {
    ...
    try {
      extract($vars);
      include(EVTemplate::template_path($template));
    } catch(Exception $e) {
      echo $e;
    }
    return ob_get_clean();
  ...
  public static function template_path($template) {
    return EVTemplate::join_paths(dirname(__FILE__), 'templates', $template . '.php');
  }
